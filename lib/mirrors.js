...
async function createGoogleAccountViaSkillsBoost(accountData) {
  // If puppeteer is not available, return account data without automation
  if (!puppeteer) {
    console.warn('Puppeteer not available, returning account data without automation');
    return {
      ...accountData,
      created: false,
      createdAt: new Date().toISOString(),
      id: Math.random().toString(36).substring(2, 15) + Date.now().toString(36),
      note: 'Account credentials generated. Puppeteer not available for automation.'
    };
  }
  ...
  try {
    ...
    console.error('Error creating account via Skills Boost:', error);
    // Return account data even if automation fails - the credentials are still valid
    // The user can manually complete the signup if needed
    return {
      ...accountData,
      created: false,
      error: error.message,
      createdAt: new Date().toISOString(),
      id: Math.random().toString(36).substring(2, 15) + Date.now().toString(36),
      note: 'Account credentials generated. Automation may have failed - credentials are still valid for manual signup.'
    };
  } finally {
    if (browser) {
      try {
